...
class ImageTextDataset(Dataset):
    def __init__(self, img_dir, tokenizer, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        if int(img_name.split('.')[0]) <= 4:
            text_input = "a sks dog"
        else:
            text_input = "a dog"
...
dataset = ImageTextDataset(img_dir="./dreambooth_dataset", tokenizer=tokenizer, transform=train_transforms)
...
writer = SummaryWriter('dream_booth_runs')
num_epochs = 20
...
StableDiffusionPipeline.save_lora_weights(
    save_directory="./fine_tuned_dreambooth_unet",
